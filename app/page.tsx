...
const PigPayLogo = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
  ...
  return (
    <div className={`${containerSizes[size]} flex items-center justify-center overflow-hidden group`}>
      <img 
        src="/logo.png" // public/logo.png を読み込みます
        alt="PigPay ロゴ"
...
export default function PigPay() {
  ...
  if (authLoading) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center">
      ...
      <p className="text-slate-400 font-bold tracking-widest animate-pulse text-sm">準備中...</p>
    </div>
  );
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-pink-50 flex items-center justify-center p-6 text-slate-800">
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-sm border-b-[12px] border-pink-200">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            {isSignUp && (
              <div className="relative">
                ...
                <input type="text" placeholder="ユーザー名" className="w-full p-4 pl-12 bg-gray-50 rounded-2xl outline-none focus:ring-2 ring-pink-400 font-bold transition-all" value={inputUsername} onChange={(e) => setInputUsername(e.target.value)} required />
              </div>
            )}
            <div className="relative">
              <Mail className="absolute left-4 top-4 text-pink-300" size={20} />
              <input type="email" placeholder="メールアドレス" className="w-full p-4 pl-12 bg-gray-50 rounded-2xl outline-none focus:ring-2 ring-pink-400 font-bold transition-all" value={email} onChange={(e) => setEmail(e.target.value)} required />
            </div>
            <div className="relative">
              <Lock className="absolute left-4 top-4 text-pink-300" size={20} />
              <input type="password" placeholder="パスワード" className="w-full p-4 pl-12 bg-gray-50 rounded-2xl outline-none focus:ring-2 ring-pink-400 font-bold transition-all" value={password} onChange={(e) => setPassword(e.target.value)} required />
            </div>
            <button className="w-full bg-pink-500 text-white py-5 rounded-2xl font-black shadow-lg hover:bg-pink-600 active:scale-95 transition-all text-lg mt-4">
              {loading ? "処理中..." : (isSignUp ? "新しく始める" : "ログインする")}
            </button>
          </form>
          <button onClick={() => setIsSignUp(!isSignUp)} className="w-full mt-8 text-xs text-pink-400 font-bold uppercase tracking-wider">
  ...
  // --- メイン画面 ---
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center pb-32 text-slate-800">
      <header className="w-full max-w-md px-6 py-6 flex justify-between items-center bg-white/50 backdrop-blur-md sticky top-0 z-30 font-bold">
        <PigPayLogo size="md" />
      ...
      <main className="w-full max-w-md px-4 pt-4">
        {view === 'home' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* 残高カード */}
            <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group text-white">
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mb-1">現在の残高</p>
                    <div className="text-5xl font-black font-mono tracking-tighter">
                      {balance.toLocaleString()} <span className="text-pink-500 text-2xl font-sans text-white">P</span>
                    </div>
                  </div>
                ...
                <div className="bg-white/10 inline-block px-4 py-2 rounded-full backdrop-blur-md">
                  <p className="font-bold text-sm italic">ID: @{username}</p>
                </div>
              </div>
            </div>
            ...
            {/* 履歴 */}
            <div>
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2 px-2 font-bold text-slate-800">
                <History size={14} /> 取引履歴
              </h2>
              <div className="space-y-3">
                {transactions.length > 0 ? transactions.map((tx) => {
                  ...
                  return (
                    <div key={tx.id} className="bg-white p-4 rounded-[2rem] shadow-sm border border-gray-50 flex justify-between items-center transition-transform hover:scale-[1.02]">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-slate-800 text-sm">{isOut ? `@${tx.receiver_username} へ` : `@${tx.sender_username} から`}</p>
                  ...
                  <div className="py-12 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-gray-100">
                    <p className="text-slate-300 font-bold">まだ履歴がありません</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {view === 'receive' && (
          <div className="animate-in zoom-in-95 text-center bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 font-bold">
            ...
            <div className="p-8 bg-slate-50 rounded-[2.5rem] inline-block border-2 border-slate-100 mb-6 shadow-inner">
              <QRCodeSVG value={username} size={180} />
            </div>
            <h3 className="text-2xl font-black text-slate-800 mb-1">@{username}</h3>
            <p className="text-slate-400 text-sm">このQRコードを相手に見せてください</p>
          </div>
        )}

        {view === 'scan' && (
          <div className="animate-in slide-in-from-bottom-8 font-bold text-slate-800">
            {!recipient ? (
              <div className="bg-slate-900 p-6 rounded-[3rem] aspect-[4/5] relative overflow-hidden shadow-2xl">
                <div className="flex justify-between items-center mb-6 relative z-10 text-white font-bold">
                  <span className="text-white/30 text-[10px] font-black uppercase tracking-widest">QRスキャン</span>
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-2xl border-b-[12px] border-pink-100 animate-in slide-in-from-right duration-300">
                <div className="flex justify-between items-center mb-10">
                  <h2 className="text-xl font-black">送金する</h2>
                ...
                <div className="bg-slate-50 p-6 rounded-3xl mb-8 flex items-center gap-4 border border-slate-100">
                  ...
                  <div>
                    <p className="text-[10px] text-slate-400 font-black uppercase">送り先</p>
                ...
                <div className="mb-10 text-center">
                  ...
                  <p className="mt-4 text-[10px] font-black text-slate-400 uppercase tracking-widest font-bold">送金可能額: {balance.toLocaleString()} P</p>
                </div>
                <button onClick={handleTransfer} disabled={isTransferring || amount <= 0 || amount > balance} className="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-black shadow-xl active:scale-95 disabled:bg-slate-100 disabled:text-slate-300 transition-all text-xl flex items-center justify-center gap-3">
                  {isTransferring ? "送金中..." : "送金を確定する"} <ChevronRight size={24} />
                </button>
              </div>
            )}
          </div>
      ...
      {/* 下部ナビゲーション */}
      <nav className="fixed bottom-8 w-[calc(100%-48px)] max-w-sm bg-white/90 backdrop-blur-xl flex justify-around items-center py-4 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] z-20 border border-white/50 font-bold">
        ...
        <button onClick={() => setView('receive')} className={`flex flex-col items-center gap-1 transition-all ${view === 'receive' ? 'text-pink-500 scale-110' : 'text-slate-300'}`}>
          ...
          <span className="text-[9px] font-black uppercase tracking-tighter">受け取る</span>
        </button>
      </nav>
    </div>
  );
